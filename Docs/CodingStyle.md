...
Follow the package's formatting rules and project conventions.

## Overview

The [Swift Format configuration](/.swift-format) is authoritative for formatting that Swift Format can enforce. The
Swift Format section explains its configuration and rules. The Project Conventions section defines requirements that
Swift Format cannot fully enforce. Formatting Exceptions explains when an exception is appropriate.

See [Formatting](/Docs/Formatting.md) for local commands, the command plugin, and continuous integration checks.
...
- `OmitExplicitReturns`: Disabled to preserve explicit `return` statements. See [Return Statements](#return-statements)
  for the convention and its Swift DSL exception.
...
- `UseSynthesizedInitializer`: Disabled because every struct must declare an explicit initializer. See
  [Initializers](#initializers) for the convention.
...
Use a period as the decimal separator. In prose and explanatory tables, group thousands with commas, such as `1,000.25`.
...
Declare access levels explicitly wherever Swift permits them. Do not omit an access modifier to rely on the default
`internal` access level. Start with `private` and widen access to `fileprivate`, `internal`, `package`, or `public` only
when needed.

Do not apply access modifiers to extensions. Declare access levels on their individual members instead, including
properties, initializers, and functions.

In standalone scripts, use `fileprivate` for file-scoped types and declarations that are only used within the script.
...
Declare at least one explicit initializer in every struct. Do not rely solely on a synthesized memberwise or default
initializer, even when Swift can generate one. If a struct requires no initialization work, declare an empty
initializer. Apply the access-control and documentation conventions to its initializers.

### Return Statements

Use an explicit `return` when returning a value from a function, closure, or computed-property getter, even when Swift
permits an implicit return. This includes single-expression and one-line declarations.

Use implicit expressions in Swift DSLs that rely on result builders, where expressions contribute to the DSL's result
rather than returning directly from the enclosing declaration.

### Documentation

Use readable, title-case link labels, such as `Security Policy` or `Package Manifest`, without file paths or extensions.

Use links starting with `/` for files and images within the repository, such as `[License](/LICENSE.md)`. Keep external
URLs and same-page anchors unchanged. DocC topic and symbol links use DocC's own syntax.
